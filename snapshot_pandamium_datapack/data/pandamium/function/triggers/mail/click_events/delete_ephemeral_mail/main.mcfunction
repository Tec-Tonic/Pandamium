...
tellraw @s [{text:"[Mail]",color:"dark_green"},{text:" Ephemeral mail was removed from your inbox. ",color:"green",extra:[{text:"[Main Menu]",color:"gold",hover_event:{action:"show_text",value:[{text:"Click to go to ",color:"gold"},{text:"Main Menu",bold:true}," page"]},click_event:{action:"run_command",command:"trigger mail set 1"}}]}]
