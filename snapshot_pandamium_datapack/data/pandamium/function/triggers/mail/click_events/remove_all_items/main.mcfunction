...
execute unless data storage pandamium.db.mail:io selected.entry.data.items[0] run return run tellraw @s [{text:"[Mail]",color:"dark_red"},{text:" You have not attached any items to this mail!",color:"red"}]
execute store result score <attached_items> variable run data get storage pandamium.db.mail:io selected.entry.data.items
...
data remove storage pandamium.db.mail:io selected.entry.data.items
#
