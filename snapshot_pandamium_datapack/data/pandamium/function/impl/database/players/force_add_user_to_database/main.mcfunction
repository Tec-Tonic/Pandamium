...
#> Pre
$execute if data storage pandamium.db.players:data username_indexes."$(username)" run tellraw @s [{text:"[admin/add_to_database]",color:"dark_red"},{text:" $(username) is already in [players]!",color:"red"}]
$execute if data storage pandamium.db.players:data username_indexes."$(username)" run return 0
$execute unless score $(username) id matches 1.. run tellraw @s [{text:"[admin/add_to_database]",color:"dark_red"},{text:" Unable to add $(username) to [players]!",color:"red"}]
$execute unless score $(username) id matches 1.. run return 0
$execute unless score $(username) playtime_ticks matches 1.. run tellraw @s [{text:"[admin/add_to_database]",color:"dark_red"},{text:" Unable to add $(username) to [players]!",color:"red"}]
...
execute if score <valid_suuid> variable matches 0 run tellraw @s [{text:"[admin/force_add_user_to_database]",color:"dark_red"},{text:" Invalid UUID! The hexadecimal uuid form is required to add a player to [players].",color:"red"}]
