...
execute if score <moved> variable matches 1 unless score @s idle.time matches 1073741824..1073741864 unless predicate pandamium:riding_rotation_dictating_vehicle run function pandamium:impl/idle/moved
...
execute at @s unless predicate {type:"minecraft:location_check",predicate:{position:{x:{min:-512,max:511.999},y:{min:-64,max:319.999},z:{min:-512,max:511.999}},dimension:"pandamium:hub"}} run return 0
# load
