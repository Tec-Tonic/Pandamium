...
$execute as @e[limit=1,x=0,type=$(entity_type),tag=$(tag)] unless predicate {type:"minecraft:entity_properties",entity:"this",predicate:{movement:{speed:{max:1}}}} run return run tag @s remove hub.minecart_stationary
