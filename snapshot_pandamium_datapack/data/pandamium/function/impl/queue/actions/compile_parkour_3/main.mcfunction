...
$execute unless data storage pandamium:temp json run return run tellraw @a[scores={id=$(source)},limit=1] [{color:"dark_red",text:"[",extra:[{storage:"pandamium:queue",nbt:"selected.entry.meta.name",interpret:true},{text:"]"}]},{text:" An error occurred whilst converting output to JSON",color:"red"}]
execute in pandamium:staff_world run data modify block 3 0 0 front_text.messages[0] set value {storage:"pandamium:temp",nbt:"json",interpret:true}
