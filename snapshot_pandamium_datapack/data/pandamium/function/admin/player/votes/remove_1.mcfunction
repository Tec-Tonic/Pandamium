...
$tellraw @a[scores={send_extra_debug_info=1..},predicate=!pandamium:matches_id] [{color:"gray",italic:true,text:"["},{color:"gray",storage:"pandamium:temp",nbt:"username",interpret:true},': admin player/votes/remove_1 {username:"$(username)"}]']
...
# feedback
$return run tellraw @s [{text:"[Admin]",color:"dark_green"},{text:" Removed 1 vote from $(username)! They now have ",color:"green",extra:[{score:{name:"$(username)",objective:"votes"}}," vote(s).\nNeed to remove any of their reward credits? ",{text:"[Click Here]",color:"aqua",click_event:{action:"suggest_command",command:"/scoreboard players remove $(username) reward_credits 1"}}]}]
