...
tag @s remove source
execute if score <summoned_entity> variable matches 1 anchored eyes run particle minecraft:witch ~ ~0.8 ~ 0.1 0.5 0.1 1 50
...
execute if score <summoned_entity> variable matches 1 run advancement grant @s only pandamium:pandamium/mob_statues/build_illusioner_statue
