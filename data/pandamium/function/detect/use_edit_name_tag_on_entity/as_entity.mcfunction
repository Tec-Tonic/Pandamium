...
## Actions
# set custom name
execute unless predicate pandamium:entity/custom_name/forced unless entity @s[predicate=pandamium:in_spawn] unless predicate {type:"minecraft:entity_properties",entity:"this",predicate:{components:{"minecraft:custom_name":{text:"#",extra:[""]}}}} run data modify storage pandamium:local functions."pandamium:detect/use_edit_name_tag_on_entity/*".dialog.actions append value {\
	"label": "Name \"#\"",\
