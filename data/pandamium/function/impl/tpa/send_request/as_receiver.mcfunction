...
execute if score <can_accept> variable matches 0 store success score <returned> variable run tellraw @a[tag=sender,limit=1] [{text:"[TPA]",color:"dark_red"},{text:" You cannot teleport to ",color:"red",extra:[{selector:"@s",color:"red"}," currently!"]}]
