...
$execute unless items entity @s $(slot) * run return run loot replace entity @s $(slot) loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"pandamium:items/custom/pale_garden_compass"}]}],modifier:[{type:"minecraft:set_components",components:{"minecraft:lodestone_tracker":{tracked:false,target:{dimension:"minecraft:overworld",pos:[I;$(x),$(y),$(z)]}}}},{type:"minecraft:set_custom_data",tag:{pandamium:{used_timestamp:$(current_gametime)}}}]}
