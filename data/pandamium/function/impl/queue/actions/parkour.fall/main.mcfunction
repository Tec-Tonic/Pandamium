...
execute store result score <total_death_points> variable in pandamium:hub if entity @e[x=0,type=area_effect_cloud,tag=parkour.death_point]
...
execute as @a[scores={parkour.has_fallen=1,parkour.checkpoint=0..,optn.parkour.restart_on_fall=0}] at @s run function pandamium:impl/parkour/actions/return_to_last_checkpoint
execute as @a[scores={parkour.has_fallen=1,parkour.checkpoint=0..,optn.parkour.restart_on_fall=1}] at @s run function pandamium:impl/parkour/actions/restart
