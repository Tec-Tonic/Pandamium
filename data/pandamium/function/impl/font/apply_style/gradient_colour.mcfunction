...
execute unless score @s font.gradient.left_colour matches 0.. run return run execute store success score <returned> variable run tellraw @s [{text:"[Font]",color:"dark_red"},{text:" You must select a left colour first!",color:"red"}]
