...
data modify storage pandamium:queue entries append value {action:'parkour.trigger_node'}
execute at @s if block ~ ~ ~ #pressure_plates as @e[type=marker,tag=parkour.node,limit=1,sort=nearest,distance=..1,nbt=!{data:{teleport:{}}}] store success score <node_at_destination> variable run data modify storage pandamium:queue entries[-1].node set from entity @s
execute if score <node_at_destination> variable matches 1 store result storage pandamium:queue entries[-1].player int 1 run scoreboard players get @s id
execute if score <node_at_destination> variable matches 0 run data remove storage pandamium:queue entries[-1]
