...
$execute store success score <entry_exists> variable store result storage pandamium:templates macro.index.index int 1 store result storage pandamium:templates macro.index__old_username__new_username.index int 1 run data get storage pandamium.db.players:data uuid_indexes."$(uuid0)_$(uuid1)_$(uuid2)_$(uuid3)"
