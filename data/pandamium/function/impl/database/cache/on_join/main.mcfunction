...
$execute unless score @s staff_rank matches 1.. run function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"kick.$(username)",id:$(id),trigger:"kick",config:"helper_punishments",target_selector:"@a[predicate=pandamium:player/min_staff_perms/helper]"}
$execute unless score @s staff_rank matches 1.. run function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"ban.$(username)",id:$(id),trigger:"ban",config:"moderator_punishments",target_selector:"@a[predicate=pandamium:player/min_staff_perms/moderator]"}
$execute unless score @s staff_rank matches 1.. run function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"jail.$(username)",id:$(id),trigger:"jail",config:{type:"jail",user_name:"$(username)"},target_selector:"@a[predicate=pandamium:player/min_staff_perms/helper]"}
$execute unless score @s staff_rank matches 1.. run function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"unjail.$(username)",id:$(id),trigger:"unjail",config:{type:"unjail",user_name:"$(username)"},target_selector:"@a[predicate=pandamium:player/min_staff_perms/helper]"}
$function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"tp.$(username)",id:$(id),trigger:"tp",config:{type:"tp",user_id:$(id)},target_selector:"@a[predicate=pandamium:player/min_staff_perms/moderator]"}
$function pandamium:impl/database/cache/on_join/add_personal_trigger/main {alias:"player_info.$(username)",id:$(id),trigger:"player_info",config:"player_info",target_selector:"@a[predicate=pandamium:player/min_staff_perms/helper]"}
