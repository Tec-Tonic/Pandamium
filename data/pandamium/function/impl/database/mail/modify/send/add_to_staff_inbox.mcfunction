...
tellraw @a[predicate=pandamium:player/min_staff_perms/helper] [{text:"[Mail]",color:"blue"},{text:" ",color:"green",extra:[{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"sender_display_name",interpret:true}]}," sent ",{text:"The Staff Team",color:"yellow"}," mail! ",{text:"[View The Staff Team's Inbox]",color:"gold",hover_event:{action:"show_text",value:[{text:"Click to see ",color:"gold"},{text:"The Staff Team's Inbox",bold:true}]},click_event:{action:"run_command",command:"trigger mail set 1000008"}}]}]
execute as @a[predicate=pandamium:player/min_staff_perms/helper] at @s run playsound block.note_block.chime master @s
