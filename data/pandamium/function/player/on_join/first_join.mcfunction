...
execute unless entity @s[predicate=pandamium:in_spawn] run tellraw @a[predicate=pandamium:player/min_staff_perms/helper] [{text:"",color:"gray"},{text:"[Staff Info]",color:"dark_gray"}," ",{selector:"@s",color:"gray"}," may have changed their username."]
# inform staff
tellraw @a[predicate=pandamium:player/min_staff_perms/helper,predicate=!pandamium:player/min_staff_perms/moderator] [{text:"",color:"gray"},{text:"[Staff Info] ",color:"dark_gray"},{selector:"@s",color:"gray"},"'s id is ",{score:{objective:"id",name:"@s"},bold:true},"! ",{text:"[→]",color:"blue",hover_event:{action:"show_text",value:[{text:"Click to teleport to ",color:"blue"},{text:"Spawn",bold:true}," in spectator mode"]},click_event:{action:"run_command",command:"trigger spawn set -1"}}]
execute store result storage pandamium:templates macro.id.id int 1 run scoreboard players get @s id
function pandamium:player/on_join/print_id_and_teleport with storage pandamium:templates macro.id
execute as @a[predicate=pandamium:player/min_staff_perms/helper] at @s run playsound entity.experience_orb.pickup master @s ~ ~ ~ 1 2 1
