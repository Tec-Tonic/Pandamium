...
execute store result score <send_command_feedback> variable run gamerule send_command_feedback
gamerule send_command_feedback false
gamemode survival @a[gamemode=adventure,scores={temp_1=0}]
gamemode adventure @a[gamemode=survival,scores={temp_1=1}]
execute if score <send_command_feedback> variable matches 1 run gamerule send_command_feedback true
# Staff World Effects
