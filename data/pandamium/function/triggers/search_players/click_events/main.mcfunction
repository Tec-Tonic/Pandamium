# load click event
scoreboard players operation <click_event_id> variable = @s search_players
...
# validate
execute if data storage pandamium.db.click_events:io selected.entry{expired:1b} run return run tellraw @s [{text:"[Search Players]",color:"dark_red"},{text:" That option has expired!",color:"red"}]
...
execute if score <owner_id> variable = @s id if data storage pandamium.db.click_events:io selected.entry{trigger:"search_players"} run scoreboard players set <valid_click_event> variable 1
...
# types
execute if data storage pandamium.db.click_events:io selected.entry.data.search run return run function pandamium:triggers/search_players/click_events/search_names/main with storage pandamium.db.click_events:io selected.entry.data
