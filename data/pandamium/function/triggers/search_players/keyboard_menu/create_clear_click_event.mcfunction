...
data modify storage do:io input set value {storage:"pandamium.db.click_events:io",nbt:"selected.entry.click_event_root",interpret:true,color:"red",extra:["[Clear]"],hover_event:{action:"show_text",value:{text:"Click to delete all characters from your search",color:"white",extra:[{text:'\n\nNew search would be: "',color:"gray",extra:['"']}]}}}
function do:text/resolve
data modify storage pandamium:temp keyboard_buttons.clear set from storage do:io output
