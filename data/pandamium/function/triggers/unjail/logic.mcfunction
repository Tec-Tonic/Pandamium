execute unless predicate pandamium:player/min_staff_perms/helper run return run tellraw @s [{text:"[Unjail]",color:"dark_red"},{text:" You do not have permission to use this trigger!",color:"red"}]
execute if score @s jailed matches 1.. unless predicate pandamium:player/min_staff_perms/senior_moderator run return run tellraw @s [{text:"[Unjail]",color:"dark_red"},{text:" You cannot use this trigger in jail!",color:"red"}]
# menus
