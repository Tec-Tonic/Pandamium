...
execute unless predicate pandamium:player/min_staff_perms/moderator unless predicate pandamium:in_spawn run return run tellraw @s [{text:"[Toggle Spectator]",color:"dark_red",hover_event:{action:"show_text",value:[{text:"Click to teleport to ",color:"yellow"},{text:"Spawn",bold:true}," in spectator mode"]},click_event:{action:"run_command",command:"trigger spawn set -1"}},{text:" Helpers may only use spectator mode at spawn!",color:"red"}]
