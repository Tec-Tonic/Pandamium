...
execute unless predicate pandamium:player/min_gameplay_perms/supporter run return run tellraw @s [{text:"[Frame Visibility]",color:"dark_red"},{storage:"pandamium:dictionary",nbt:"triggers.supporter_only_trigger",interpret:true},{text:"\n This command would let you make item frames invisible!",color:"gray"}]
