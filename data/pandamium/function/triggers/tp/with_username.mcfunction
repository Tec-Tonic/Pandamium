...
execute if score <target_is_online> variable matches 1 run return run tellraw @s [{text:"[TP]",color:"dark_green"},{text:" Teleported to ",color:"green",extra:[{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"display_name",interpret:true}]}," (in spectator mode)!"]}]
# target is offline
$execute unless score $(username) last_position.x = $(username) last_position.x run return run tellraw @s [{text:"[TP]",color:"dark_red"},{text:" ",color:"red",extra:[{storage:"pandamium:temp",nbt:"display_name",interpret:true}," has no known last position!"]}]
...
tellraw @s [{text:"[TP]",color:"dark_green"},{text:" Teleported to ",color:"green",extra:[{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"display_name",interpret:true}]},"'s last position (in spectator mode)!"]}]
