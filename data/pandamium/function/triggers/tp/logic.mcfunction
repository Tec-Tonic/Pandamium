execute unless predicate pandamium:player/min_staff_perms/moderator run return run tellraw @s [{text:"[TP]",color:"dark_red"},{text:" You do not have permission to use this trigger!",color:"red"}]
...
execute unless data storage pandamium.db.players:io selected run return run tellraw @s [{text:"[TP]",color:"dark_red"},{text:" Could not find a player with ID ",color:"red",extra:[{score:{name:"@s",objective:"tp"}},{text:"!"}]}]
