...
def get_advancement_data(advancement_id: str, fallback: str = None, sort_requirements: bool = False) -> None:
    ...
    # <datapack>/data/<namespace>/advancement/<name>.json — works for both "minecraft:foo/bar" and
    # "pandamium:path/to/name" styles regardless of the path separator this script is run from.
    data_pack_root_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))))

    if fallback == None:
        advancement_path = os.path.join(data_pack_root_path, "data", advancement_namespace, "advancement", *advancement_name.split("/")[:-1], f'{advancement_name.split("/")[-1]}.json')
    ...
    if advancement_id == "pandamium:pandamium/mini_blocks/craft_every_mini_block":
        for criterion in advancement_data["criteria"]:
            with open(os.path.join(data_pack_root_path, "data", "pandamium", "recipe", "mini_blocks", f"{criterion}.json"),"r") as file:
        ...
        for mob_head_entity_type in os.listdir(os.path.join(data_pack_root_path, "data", "pandamium", "loot_table", "items", "heads", "mobs")):
            for mob_head_type in os.listdir(os.path.join(data_pack_root_path, "data", "pandamium", "loot_table", "items", "heads", "mobs", mob_head_entity_type)):
        ...
        for criterion in advancement_data["criteria"]:
            ...
            with open((file_path := os.path.join(data_pack_root_path, "data", "pandamium", "loot_table", "items", "heads", "mobs", mob_head_entity_type, f"{mob_head_type}.json")),"r") as file:
                file_contents = json.loads(file.read())
                entries = file_contents["pools"][0]["entries"][0]
                mods = entries.get("modifier") or entries.get("functions") or []
                # find the set_name and set_components/attributes entries (26.3 uses "modifier")
                name = mods[-1]["name"][1] if len(mods[-1].get("name", [])) > 1 else {"text": mob_head_type}
                if "bold" in name:
                    del name["bold"]
                profile = None
                for m in mods:
                    if m.get("type") in ("minecraft:set_components", "set_components") and "minecraft:profile" in m.get("components", {}):
                        profile = m["components"]["minecraft:profile"]
                        break
                advancement_data["criteria"][criterion]["__name__"] = name
                advancement_data["criteria"][criterion]["__item__"] = {"id":"minecraft:player_head","components":{"minecraft:profile":profile}} if profile else {"id":"minecraft:player_head"}
    ...
    elif advancement_id == "minecraft:husbandry/bred_all_animals":
        # Each criterion is a vanilla entity; name it with the entity's translated name
        # and give it the matching spawn egg so the list shows the creature's icon.
        for criterion in advancement_data["criteria"]:
            entity = criterion.split(":")[-1]
            advancement_data["criteria"][criterion]["__name__"] = {"translate":f"entity.minecraft.{entity}"}
            advancement_data["criteria"][criterion]["__item__"] = {"id":f"minecraft:{entity}_spawn_egg"}
    elif advancement_id == "minecraft:husbandry/whole_pack":
        # Wolf variants: show the wolf spawn egg icon; name is the plain variant id.
        for criterion in advancement_data["criteria"]:
            variant = criterion.split(":")[-1]
            advancement_data["criteria"][criterion]["__name__"] = {"translate":f"entity.minecraft.wolf.{variant}"}
            advancement_data["criteria"][criterion]["__item__"] = {"id":"minecraft:wolf_spawn_egg"}
    elif advancement_id == "minecraft:husbandry/complete_catalogue":
        # Cat variants: show the cat spawn-egg icon + translated cat-variant name.
        for criterion in advancement_data["criteria"]:
            variant = criterion.split(":")[-1]
            advancement_data["criteria"][criterion]["__name__"] = {"translate":f"advancements.husbandry.complete_catalogue.{variant}"}
            advancement_data["criteria"][criterion]["__item__"] = {"id":"minecraft:cat_spawn_egg"}
    elif advancement_id == "minecraft:adventure/trim_with_all_exclusive_armor_patterns":
        # Each criterion contains the armour-trim smithing template recipe name in its ID.
        for criterion in advancement_data["criteria"]:
            template = criterion.split(":")[-1].replace("_armor_trim_smithing_template_smithing_trim","") + "_armor_trim_smithing_template"
            advancement_data["criteria"][criterion]["__name__"] = {"translate":f"item.minecraft.{template}"}
            advancement_data["criteria"][criterion]["__item__"] = {"id":f"minecraft:{template}"}
    else:
        for criterion in advancement_data["criteria"]:
    ...
    with open(file_name,"w") as file:
        ...
        for criteria_required in advancement_requirements:
            ...
            predicate = (
                '{type:"minecraft:entity_properties",entity:"this",predicate:{"minecraft:type_specific/player":{advancements:{"%s":%s}}}}'
                % (
                    advancement_id,
...
get_advancement_data("minecraft:adventure/kill_all_mobs",
  fallback=r"""{"parent":"minecraft:adventure/kill_a_mob","criteria":{"minecraft:blaze":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:blaze"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:bogged":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:bogged"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:breeze":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:breeze"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:camel_husk":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:camel_husk"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:cave_spider":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:cave_spider"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:creaking":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:creaking"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:creeper":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:creeper"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:drowned":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:drowned"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:elder_guardian":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:elder_guardian"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:ender_dragon":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:ender_dragon"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:enderman":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:enderman"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:endermite":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:endermite"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:evoker":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:evoker"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:ghast":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:ghast"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:guardian":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:guardian"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:hoglin":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:hoglin"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:husk":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:husk"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:magma_cube":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:magma_cube"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:parched":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:parched"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:phantom":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:phantom"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:piglin":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:piglin"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:piglin_brute":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:piglin_brute"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:pillager":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:pillager"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:ravager":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:ravager"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:shulker":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:shulker"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:silverfish":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:silverfish"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:skeleton":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:skeleton"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:slime":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:slime"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:spider":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:spider"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:stray":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:stray"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:vex":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:vex"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:vindicator":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:vindicator"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:witch":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:witch"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:wither":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:wither"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:wither_skeleton":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:wither_skeleton"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zoglin":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zoglin"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zombie":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zombie"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zombie_horse":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zombie_horse"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zombie_nautilus":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zombie_nautilus"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zombie_villager":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zombie_villager"}}]},"trigger":"minecraft:player_killed_entity"},"minecraft:zombified_piglin":{"conditions":{"entity":[{"condition":"minecraft:entity_properties","entity":"this","predicate":{"type":"minecraft:zombified_piglin"}}]},"trigger":"minecraft:player_killed_entity"}},"display":{"description":{"translate":"advancements.adventure.kill_all_mobs.description"},"frame":"challenge","icon":{"count":1,"id":"minecraft:diamond_sword"},"title":{"translate":"advancements.adventure.kill_all_mobs.title"}},"requirements":[["minecraft:blaze"],["minecraft:bogged"],["minecraft:breeze"],["minecraft:camel_husk"],["minecraft:cave_spider"],["minecraft:creaking"],["minecraft:creeper"],["minecraft:drowned"],["minecraft:elder_guardian"],["minecraft:ender_dragon"],["minecraft:enderman"],["minecraft:endermite"],["minecraft:evoker"],["minecraft:ghast"],["minecraft:guardian"],["minecraft:hoglin"],["minecraft:husk"],["minecraft:magma_cube"],["minecraft:parched"],["minecraft:phantom"],["minecraft:piglin"],["minecraft:piglin_brute"],["minecraft:pillager"],["minecraft:ravager"],["minecraft:shulker"],["minecraft:silverfish"],["minecraft:skeleton"],["minecraft:slime"],["minecraft:spider"],["minecraft:stray"],["minecraft:vex"],["minecraft:vindicator"],["minecraft:witch"],["minecraft:wither_skeleton"],["minecraft:wither"],["minecraft:zoglin"],["minecraft:zombie_villager"],["minecraft:zombie"],["minecraft:zombie_horse"],["minecraft:zombified_piglin"],["minecraft:zombie_nautilus"]],"rewards":{"experience":100},"sends_telemetry_event":true}""",
...
get_advancement_data("minecraft:husbandry/bred_all_animals")
get_advancement_data("minecraft:husbandry/whole_pack")
get_advancement_data("minecraft:husbandry/complete_catalogue")
get_advancement_data("minecraft:adventure/trim_with_all_exclusive_armor_patterns")
get_advancement_data("minecraft:nether/explore_nether")
