...
execute if score @s mail matches 1000004 if predicate pandamium:player/min_staff_perms/senior_moderator if score @s send_extra_debug_info matches 2.. run return run function pandamium:triggers/mail/print_inbox_outbox_menu/main {type: "server_outbox", self: true}
...
execute if score @s mail matches 1000008 if predicate pandamium:player/min_staff_perms/helper run return run function pandamium:triggers/mail/print_inbox_outbox_menu/main {type: "staff_inbox", self: true}
...
execute if score @s mail matches 1000201 unless predicate pandamium:player/min_staff_perms/helper run return run function pandamium:triggers/mail/create_mail/main
