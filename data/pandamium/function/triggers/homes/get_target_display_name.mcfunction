...
execute if score <target_is_online> variable matches 0 run data modify storage pandamium:temp target set value [{storage:"pandamium.db.players:io",nbt:"selected.entry.username",interpret:true}," (offline)"]
