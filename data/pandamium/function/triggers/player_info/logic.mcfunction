...
execute if score @s player_info matches -1 run scoreboard players operation @s player_info = @a[predicate=!pandamium:player/min_staff_perms/helper,distance=..200,limit=1] id
...
execute if score @s player_info matches -1999999..-1000001 if predicate pandamium:player/min_staff_perms/moderator run return run function pandamium:triggers/player_info/print_teleport_history_menu/main with storage pandamium:temp arguments
execute if score @s player_info matches -1999999..-1000001 unless predicate pandamium:player/min_staff_perms/moderator run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" You do not have permission to access that data!",color:"red"}]
# print mail menus
execute if score @s player_info matches -2999999..-2000001 if predicate pandamium:player/min_staff_perms/senior_moderator run return run function pandamium:triggers/mail/print_inbox_outbox_menu/main {type: "inbox", self: false}
execute if score @s player_info matches -3999999..-3000001 if predicate pandamium:player/min_staff_perms/senior_moderator run return run function pandamium:triggers/mail/print_inbox_outbox_menu/main {type: "outbox", self: false}
execute if score @s player_info matches -3999999..-2000001 unless predicate pandamium:player/min_staff_perms/senior_moderator run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" You do not have permission to access that data!",color:"red"}]
...
execute if score @s player_info matches -8999999..-8000001 if predicate pandamium:player/min_staff_perms/moderator run return run function pandamium:triggers/player_info/tp_last_death_location/main
execute if score @s player_info matches -8999999..-8000001 unless predicate pandamium:player/min_staff_perms/moderator run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" You do not have permission to perform that action!",color:"red"}]
...
# take items on join
execute if score @s player_info matches -10999999..-10000001 if predicate pandamium:player/min_staff_perms/moderator run return run function pandamium:triggers/player_info/take_items_on_join/main with storage pandamium:temp arguments
execute if score @s player_info matches -10999999..-10000001 unless predicate pandamium:player/min_staff_perms/moderator run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" You do not have permission to perform that action!",color:"red"}]
# else
