scoreboard players operation <click_event_id> variable = @s player_info
...
execute unless score <owner_id> variable = @s id run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" That is not a valid option!",color:"red"}]
execute unless data storage pandamium.db.click_events:io selected.entry{trigger:"player_info"} run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" That is not a valid option!",color:"red"}]
# currently only "teleport" type exists
execute unless data storage pandamium.db.click_events:io selected.entry.data{type:"teleport"} run return run tellraw @s [{text:"[Player Info]",color:"dark_red"},{text:" That is not a valid option!",color:"red"}]
...
function pandamium:triggers/player_info/do_dynamic_click_event/with_username with storage pandamium.db.click_events:io selected.entry.data
function pandamium:utils/get/dimension_name/from_score
tellraw @s [{text:"[Player Info]",color:"dark_green"},{text:" Teleported to ",color:"green",extra:[{score:{name:"<tp_x>",objective:"variable"},color:"aqua",extra:[" ",{score:{name:"<tp_y>",objective:"variable"}}," ",{score:{name:"<tp_z>",objective:"variable"}}]}," in ",{storage:"pandamium:temp",nbt:"dimension_name",interpret:true,color:"aqua"},"!"]}]
