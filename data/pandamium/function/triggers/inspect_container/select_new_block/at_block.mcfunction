...
execute unless data storage pandamium:containers items[0] run return run tellraw @s [{text:"[Containers] ",color:"dark_red"},{text:"The ",color:"red",extra:[{nbt:"containers.display_id",storage:"pandamium:temp",interpret:true,bold:true},{text:" at "},{nbt:"containers.nbt.x",storage:"pandamium:temp",bold:true,extra:[{text:" "},{nbt:"containers.nbt.y",storage:"pandamium:temp"},{text:" "},{nbt:"containers.nbt.z",storage:"pandamium:temp"}]},{text:" has no items in it!"}]}]
