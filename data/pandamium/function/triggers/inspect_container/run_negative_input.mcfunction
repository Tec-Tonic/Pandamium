...
scoreboard players operation <slot> variable -= @s inspect_container
data remove storage pandamium:containers source
execute if score <returned> variable matches 0 summon marker run function pandamium:triggers/inspect_container/reselect_block/as_marker
