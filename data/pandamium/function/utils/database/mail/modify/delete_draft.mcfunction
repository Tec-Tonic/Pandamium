...
execute unless data storage pandamium.db.mail:io selected.entry{draft:1b} run return fail
execute if data storage pandamium.db.mail:io selected.entry.data.items[0] run return fail
# remove "saved as draft"
data modify storage pandamium.db.mail:io selected.entry.draft set value 0b
## Sender
# load from id
...
# remove from drafts
function pandamium:impl/database/mail/modify/delete_draft/remove_from_drafts with storage pandamium.db.mail:io selected.entry
