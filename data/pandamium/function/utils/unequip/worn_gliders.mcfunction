...
execute if items entity @s armor.feet *[glider] if predicate pandamium:player/can_pick_up_any_item in pandamium:staff_world run function pandamium:impl/unequip/feet
execute if items entity @s armor.legs *[glider] if predicate pandamium:player/can_pick_up_any_item in pandamium:staff_world run function pandamium:impl/unequip/legs
execute if items entity @s armor.chest *[glider] if predicate pandamium:player/can_pick_up_any_item in pandamium:staff_world run function pandamium:impl/unequip/chest
execute if items entity @s armor.head *[glider] if predicate pandamium:player/can_pick_up_any_item in pandamium:staff_world run function pandamium:impl/unequip/head
...
data modify storage pandamium.db.mail:io selected.entry.data.message set value "Your inventory was too full to unequip the elytra that you were wearing.\n\nThe item has been safely moved here for you to reclaim later."
...
execute if items entity @s armor.feet *[glider] in pandamium:staff_world run function pandamium:utils/database/mail/modify/attach_item {from:'block 5 0 0 item'}
...
execute if items entity @s armor.legs *[glider] in pandamium:staff_world run function pandamium:utils/database/mail/modify/attach_item {from:'block 5 0 0 item'}
...
execute if items entity @s armor.chest *[glider] in pandamium:staff_world run function pandamium:utils/database/mail/modify/attach_item {from:'block 5 0 0 item'}
...
execute if items entity @s armor.head *[glider] in pandamium:staff_world run function pandamium:utils/database/mail/modify/attach_item {from:'block 5 0 0 item'}
